...
{
    public partial class PrintClassListForm : Form
    {
        public PrintClassListForm()
        {
            InitializeComponent();
            FillClass();
            FillAcademicYear();
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string Query = "SELECT DISTINCT S.IDNO AS IDNO, S.FullName AS FullName, S.DPob AS DPob, S.Sex AS Sex, S.GuardianName AS GuardianName, S.GuardianTel AS GuardianTel, C.ClassName AS Class, T.ClassRoomName AS Classroom, A.AcademicYear AS AcademicYear, E.FullName AS FullNameE FROM student S, tuitionfees T, academicyear A, class C, assignclass N, employee E WHERE T.AcademicYearID = A.AcademicYearID AND T.classID = C.ClassID AND T.StudentID = S.StudentID AND N.ClassID = T.ClassID AND N.EmployeeID = E.EmployeeID AND T.AcademicYearID = N.AcademicYearID AND (T.ClassID = '" + this.lblClassID.Text + "' && T.ClassroomName LIKE '%" + this.cmbClassRoomName.Text + "%' && T.AcademicYearID = '" + this.lblAcademicYearID.Text + "')";//+
                ...
                ClassListCR rpt = new ClassListCR();
                MyAdapter.Fill(DS, "ClassList");
