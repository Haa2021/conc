...
{
    public partial class PrintPupilWithDisabilityForm : Form
    {
        public PrintPupilWithDisabilityForm()
        {
            InitializeComponent();
            FillClass();
            FillAcademicYear();
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string Query = "SELECT S.IDNO AS IDNO, S.FullName AS FullName, M.Disabled AS Disabled, M.NatureOfDisability AS NatureOfDisability, M.DegreeOfDisability AS DegreeOfDisability," +
                    "M.InvalidityCardNo AS InvalidityCardNo, M.DateOfIssue AS DateOfIssue, M.PlaceOfIssue AS PlaceOfIssue, M.InvalidityCardNo AS InvalidityCardNo, " +
                    "C.ClassName AS Class, T.ClassRoomName AS Classroom, A.AcademicYear AS AcademicYear " +
                    "FROM student S, tuitionfees T, academicyear A, class C, studentmedicaldetails M  " +
                    "WHERE M.StudentID = S.StudentID AND T.AcademicYearID = A.AcademicYearID AND T.classID = C.ClassID AND T.StudentID = S.StudentID AND (T.ClassID LIKE '%" + this.lblClassID.Text + "%' && T.ClassroomName LIKE '%" + this.cmbClassRoomName.Text + "%' && T.AcademicYearID LIKE '%" + this.lblAcademicYearID.Text + "%') AND M.Disabled = 'Yes' " +
                ...
                DisabilityCR rpt = new DisabilityCR();
                MyAdapter.Fill(DS, "Disability");
